 ...
 */
public class OnlineMultilayerPerceptron 
   ...
   * The constructor.
   */
  public OnlineMultilayerPerceptron() {
    m_instances = null;
  ...
  public void buildClassifier(Instances i) throws Exception {
    ...
    //m_instances.randomize(m_random); //commented by me
	...
	if (!m_currentInstance.classIsMissing()) {
	  ...
	  /*for(int t = 0; t < this.m_neuralNodes.length; t++){
	    System.out.print("Node "+t+": ");
	    double[] nodeweights = ((NeuralNode)this.m_neuralNodes[t]).getWeights();
	    for(int p = 0; p < 3; p++)
	      System.out.println(nodeweights[p] + "\t");
	  }
	  System.out.println();*/
	}
  ...
  public void initMLP(Instance firstIns){
    m_instances = this.getInstances(firstIns);
    m_epoch = 0;
    m_error = 0;
    m_currentInstance = firstIns;
    m_controlPanel = null;
    m_nodePanel = null;
  ...
  public static double temp_right = 0;
  public static double numTrainedInst_weights = 0;
  @Override
  public void trainOnInstanceImpl(Instance inst) {
    ...
    int numInVal = 0;
    double tempRate;
    double right = temp_right;
    double totalWeight=numTrainedInst_weights;
    double origRate = m_learningRate; //only used for when reset
    for (int noa = numInVal; noa < m_instances.numInstances(); noa++) {
	if (!m_instances.instance(noa).classIsMissing()) {
	  totalWeight += m_instances.instance(noa).weight();
	  numTrainedInst_weights = totalWeight;
	}
    }
